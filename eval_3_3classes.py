...
tf.flags.DEFINE_string("checkpoint_dir_rnn", "runs/1552660061/checkpoints", "Checkpoint directory from training run")
tf.flags.DEFINE_string("checkpoint_dir_cnn", "cnn_runs/1552660711/checkpoints", "Checkpoint directory from training run")
...
def eval():
    with tf.device('/gpu:0'):
        ...
        x_text, y = data_helpers.load_data_and_labels2(FLAGS.rank1_dir, FLAGS.rank2_dir, FLAGS.rank3_dir)
...
def change_ensemble_weight(rnn_logits_list, cnn_logits_list, SVM_pred_list, y_eval):
    ...
    for i in range(11):
        ...
        for j in range(11-i):
            ...
            for pred_rnn, pred_cnn, pred_SVM, label in zip(rnn_logits_list, cnn_logits_list, SVM_pred_list, y_eval):
                ...
                final_pos0 = pred_rnn[0] * ratio_RNN + pred_cnn[0] * ratio_CNN + pred_SVM[0] * ratio_SVM
                final_pos1 = pred_rnn[1] * ratio_RNN + pred_cnn[1] * ratio_CNN + pred_SVM[1] * ratio_SVM
                final_pos2 = pred_rnn[2] * ratio_RNN + pred_cnn[2] * ratio_CNN + pred_SVM[2] * ratio_SVM
                final_pred = [final_pos0, final_pos1, final_pos2]
    ...
    # analyse result:
    each_result = len(ensemble_label_list) / 3
    i = 0
    true_positive = 0
    true_negative = 0
    false_negative = 0
...
def change_keyword_weight(weight_list, keyword_pred, neural_pred, word_num, keyword_num, first3_keyword_num, label):
    ...
    ensemble_pred_pos0 = float(weight_list[word_num][keyword_num][first3_keyword_num]) * float(keyword_pred[0]) + neural_pred[0]
    ensemble_pred_pos1 = float(weight_list[word_num][keyword_num][first3_keyword_num]) * float(keyword_pred[1]) + neural_pred[1]
    ensemble_pred_pos2 = float(weight_list[word_num][keyword_num][first3_keyword_num]) * float(keyword_pred[2]) + neural_pred[2]
    ensemble_pred = [ensemble_pred_pos0, ensemble_pred_pos1, ensemble_pred_pos2]
    final_pred = ensemble_pred.index(max(ensemble_pred))
    keyword_pred = keyword_pred.index(max(keyword_pred))
    tmp_pred = [neural_pred[0], neural_pred[1], neural_pred[2]]
...
def final_pred(neural_pred, keywords_pred, weight_list, labels, headlines_test, keywords_list):
    ...
    for pred1, pred2, label, headlines in zip(neural_pred, keywords_pred, labels, headlines_test):
        ...
        ensemble_pred_pos0 = float(weight_list[word_num][keyword_num][First3_num]) * float(pred2[1]) + pred1[0]
        ensemble_pred_pos1 = float(weight_list[word_num][keyword_num][First3_num]) * float(pred2[2]) + pred1[1]
        ensemble_pred_pos2 = float(weight_list[word_num][keyword_num][First3_num]) * float(pred2[3]) + pred1[2]
        ensemble_pred = [ensemble_pred_pos0, ensemble_pred_pos1, ensemble_pred_pos2]
...
def main(_):
    ...
    # analyse result:
    each_result = len(rnn_logits_list) / 3
    i = 0
    true_positive = 0
    true_negative = 0
    false_negative = 0
    ...
    # analyse result:
    each_result = len(cnn_logits_list) / 3
    i = 0
    true_positive = 0
    true_negative = 0
    false_negative = 0
    ...
    # ensemble with SVM
    SVM_pred_list = SVM_3classes.SVM_eval()
