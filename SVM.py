...
def readheadlines():
    try:
        with open("data/keywords_analysis/headlines_list") as train_headline, open("data/keywords_analysis/test_headlines") as test_headline, open("data/keywords_analysis/labels_list") as train_label, open("data/keywords_analysis/test_labels") as test_label, open("data/keywords_analysis/placeholder.txt") as ph:
            clean_train_h = []
            clean_test_h = []
            clean_train_l = []
            clean_test_l = []
...
def SVM_eval():
    try:
        ...
        # try TfidfVectorizer to see if it's improved
        # text_clf = Pipeline([('vect', TfidfVectorizer()), ('tfidf', TfidfTransformer()), ('clf', SVC(C=0.99, kernel = 'linear', probability=True))])
        text_clf = Pipeline([('vect', TfidfVectorizer()), ('tfidf', TfidfVectorizer()), ('clf', SVC(C=0.99, kernel = 'linear', probability=True))])
